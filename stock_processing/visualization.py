...
class CompanyVisualization:
    ...
    def balance_sheet(self):
        ...
        fig = make_subplots(rows=2, cols=1, subplot_titles=('Ngắn hạn', 'Dài hạn'), vertical_spacing=0.15,
        ...
        fig.add_trace(go.Bar(x=balance_sheet.index, y=balance_sheet['debt'], name='Tổng nợ',
...
class Comparision:
    ...
    def company_roe_roa_comparison(self):
        ...
        fig = make_subplots(rows=3, cols=1, shared_xaxes=True,  # 3 rows for subplots, shared x-axis
                            subplot_titles=(f"ROE - {self.ticker} vs Ngành", 
                                            f"ROA - {self.ticker} vs Ngành", 
                                            f"Tỷ lệ ROE/ROA - {self.ticker}"),
                            vertical_spacing=0.1)

        # Subplot 1: ROE comparison
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['roe'],
                                name="ROE ngành", line=dict(color='#00ffff', width=2), mode='lines+markers'), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['roe'],
                                name="ROE công ty", line=dict(color='#ff6b6b', width=2), mode='lines+markers'), row=1, col=1)

        # Subplot 2: ROA comparison
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['roa'],
                                name='ROA ngành', line=dict(color='#00ffff', width=2), mode='lines+markers'), row=2, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['roa'],
                                name='ROA công ty', line=dict(color='#ff6b6b', width=2), mode='lines+markers'), row=2, col=1)
        ...
        title = f"So sánh ROE và ROA - {self.ticker}"
        fig.update_layout(
            title=title,
    ...
    def company_vs_industry_pe_pb(self):
        ...
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True,  # Shared x-axes
                            subplot_titles=(f"PE - {self.ticker} vs Ngành",
                                            f"PB - {self.ticker} vs Ngành"),
        ...
        title = f"So sánh PE và PB - {self.ticker} vs Ngành"
    ...
    def equity_ratios_comparison(self):
        ...
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                            subplot_titles=(f"Vốn chủ sở hữu/Nợ - {self.ticker} vs Ngành",
                                            f"Vốn chủ sở hữu/Tổng tài sản - {self.ticker} vs Ngành"),
                            vertical_spacing=0.1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['equity_on_liability'], mode='lines+markers',
                                    name='Ngành Vốn chủ sở hữu/Nợ', line=dict(color='#00ffff', width=2)), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, 
                                    y=self.ticker_ratio_df['equity_on_liability'], mode='lines+markers',
                                    name=f'{self.ticker} Vốn chủ sở hữu/Nợ', line=dict(color='#ff6b6b', width=2)), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['equity_on_total_asset'], mode='lines+markers',
                                    name='Ngành Vốn chủ sở hữu/Tổng tài sản', line=dict(color='#00ffff', width=2)), row=2, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['equity_on_total_asset'], mode='lines+markers',
                                    name=f'{self.ticker} Vốn chủ sở hữu/Tổng tài sản', line=dict(color='#ff6b6b', width=2)), row=2, col=1)

        # Update layout (common for both subplots)
        title = f"So sánh tỷ lệ vốn chủ sở hữu - {self.ticker} vs Ngành"
        update_layout_dark(fig, title, yaxis_title="Vốn chủ sở hữu/Nợ", yaxis3_title="Vốn chủ sở hữu/Tổng tài sản")
...
class CompanyComparison(Comparision):
    ...
    def profit_margins_comparison(self):
        ...
        fig = make_subplots(rows=3, cols=1, shared_xaxes=True,
                            subplot_titles=(f"Tỷ suất Lợi nhuận Gộp - {self.ticker} vs Ngành",
                                            f"Tỷ suất Lợi nhuận Hoạt động - {self.ticker} vs Ngành",
                                            f"Tỷ suất Lợi nhuận Sau Thuế - {self.ticker} vs Ngành"),
                            vertical_spacing=0.1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['gross_profit_margin'], mode='lines+markers',
                                 name='Tỷ suất Lợi nhuận Gộp Ngành', line=dict(color='#00ffff', width=2)), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['gross_profit_margin'], mode='lines+markers',
                                 name=f'Tỷ suất Lợi nhuận Gộp {self.ticker}', line=dict(color='#ff6b6b', width=2)), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['operating_profit_margin'], mode='lines+markers',
                                 name='Tỷ suất Lợi nhuận Hoạt động Ngành', line=dict(color='#00ffff', width=2)), row=2, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['operating_profit_margin'], mode='lines+markers',
                                 name=f'Tỷ suất Lợi nhuận Hoạt động {self.ticker}', line=dict(color='#ff6b6b', width=2)), row=2, col=1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['post_tax_margin'], mode='lines+markers',
                                 name='Tỷ suất Lợi nhuận Sau Thuế Ngành', line=dict(color='#00ffff', width=2)), row=3, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['post_tax_margin'], mode='lines+markers',
                                 name=f'Tỷ suất Lợi nhuận Sau Thuế {self.ticker}', line=dict(color='#ff6b6b', width=2)), row=3, col=1)

        # Update layout
        title = f"So sánh Tỷ suất Lợi nhuận - {self.ticker} vs Ngành"
        update_layout_dark(fig, title, yaxis_title="Tỷ suất Lợi nhuận Gộp", 
                           yaxis3_title="Tỷ suất Lợi nhuận Hoạt động")
        fig.update_layout(yaxis5=dict(title="Tỷ suất Lợi nhuận Sau Thuế", gridcolor='rgba(128, 128, 128, 0)', color='white', overlaying='y', side='right'))
    ...
    def days_inventory_and_days_payable(self):
        ...
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                            subplot_titles=(f"Số ngày tồn kho - {self.ticker} vs Ngành",
                                            f"Số ngày phải trả - {self.ticker} vs Ngành"),
                            vertical_spacing=0.1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['days_inventory'], mode='lines+markers',
                                    name='Số ngày tồn kho ngành', line=dict(color='#00ffff', width=2)), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['days_inventory'], mode='lines+markers',
                                    name=f'Số ngày tồn kho {self.ticker}', line=dict(color='#ff6b6b', width=2)), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(x=self.industry_ratio_df.index, y=self.industry_ratio_df['days_payable'], mode='lines+markers',
                                    name='Số ngày phải trả ngành', line=dict(color='#00ffff', width=2)), row=2, col=1)
        fig.add_trace(go.Scatter(x=self.ticker_ratio_df.index, y=self.ticker_ratio_df['days_payable'], mode='lines+markers',
                                    name=f'Số ngày phải trả {self.ticker}', line=dict(color='#ff6b6b', width=2)), row=2, col=1)

        # Update layout
        title = f"So sánh số ngày tồn kho và số ngày phải trả - {self.ticker} vs Ngành"
        update_layout_dark(fig, title, yaxis_title="Số ngày tồn kho", yaxis3_title="Số ngày phải trả")
...
class BankVisualization:
    ...
    def income_statement(self):
        ...
        # Subplot 2: Revenue & Operating Expenses, Post-tax Profit
